...
Fos.TConsignGrid = function() {
	var bp={_A:'TCON_Q',_mt:'xml',versionFlag:2};
	...
	this.expeSum=function(){
		...
		store.reload({params:{start:0},callback:function(){sum();}});
	}
	...
	var cm=new Ext.grid.ColumnModel({
		columns:[
			...
			{header:boldFont('现付'),dataIndex:'expenseXff',renderer:function(v,m,r){m.css='green-b';return '￥'+v;}},
			{header:boldFont('到付'),dataIndex:'expenseDff',renderer:function(v,m,r){m.css='green-b';return '￥'+v;}},
			{header:boldFont('回单付'),dataIndex:'expenseHdf',renderer:function(v,m,r){m.css='green-b';return '￥'+v;}},
			{header:boldFont('月结'),dataIndex:'expenseYjf',renderer:function(v,m,r){m.css='green-b';return '￥'+v;}},
	...
	var submDate=new Ext.form.DateField({fieldLabel:'收提交日期',format:DATEF,width:130,height:18,anchor:'95%'});
	var submDateTo=new Ext.form.DateField({fieldLabel:'至',format:DATEF,width:130,height:18,anchor:'95%'});
	var submDate2=new Ext.form.DateField({fieldLabel:'付提交日期',format:DATEF,width:130,height:18,anchor:'95%'});
	var submDate2To=new Ext.form.DateField({fieldLabel:'至',format:DATEF,width:130,height:18,anchor:'95%'});
...
Fos.TConsTab = function(p,listStore){
    ...
    var cm=new Ext.grid.ColumnModel({columns:[new Ext.grid.RowNumberer(),sm,
	  	...
	  	{header:'货名',dataIndex:'cargoName',width:120,editor:new Ext.form.TextField()},
	  	{header:C_PACK,dataIndex:'packName',width:120,
        ...
        {header:'保险费(承运人)',dataIndex:'premiumExpenseProvider',width:120,
              ...
              editable:false,//保险费由系统自动计算 产生 ，无需录入
              editor:new Ext.form.NumberField({decimalPrecision:4,allowBlank:false,emptyText:'',invalidText:''}),hidden:true},
	  	 {header:C_REMARKS,dataIndex:'remarks',width:300,editor:new Ext.form.TextField()}
	  	],defaults:{sortable:false,width:100}});
    	
	this.addConsignCargo = function(){
		var tc = new TConsignCargo({uuid:HTUtil.UUID(32),version:'0',rowAction:'N'
			//packagesRemainder:'0.00',grossWeightRemainder:'0.00',measurementRemainder:'0.00'
		});
		grid.stopEditing();
    	store.insert(0,tc);
	...
	this.del = function(){
		...
		if(r.length){
			for(var i=0;i<r.length;i++){
				...
				if(r[i].get('status')=='1'){
					XMG.alert(SYS,'货名：'+r[i].get('cargoName')+'\n已经被派车,不能删除！');
					return ;
				}
				store.remove(r[i]);
			}
		}else{
			Ext.MessageBox.alert(SYS,M_R_P);
		}
		this.reCalculate();//删除记录后重新计算合计项目
	};
	...
	var grid = new Ext.grid.EditorGridPanel({title:'',height:200,iconCls:'gen',region:'center',
		clicksToEdit:2,autoScroll:true,sm:sm,cm:cm,store:store,
		/*listeners:{scope:this,
			afteredit:function(e){
				var f=e.field;
	    		if(f=='packages'||f=='grossWeight'||f=='measurement'||
	    		   f=='grossWeightProvider'||f=='measurementProvider'||
	    		   f=='premiumValueProvider'||f=='premiumExpenseProvider'){
	    			reCalculate();
	    		}
    		}
    	},*/
    ...
    var cboFeedbackWay = new Ext.form.ComboBox({fieldLabel:'返单类型',anchor:'95%',
		...
		enableKeyEvents:true,tabIndex:13,editable:false,
	...
	var cboConsBelong = new Ext.form.ComboBox({fieldLabel:'委托归属',anchor:'95%',
		name:'consBelong',value:p.get('consBelong'),itemCls:'required',
		tabIndex:18,store:HTStore.consBelong,displayField:'N',valueField:'C',editable:false,
    ...
    var txtCargoStatus = new Ext.form.TextArea({fieldLabel:'货物状况',anchor:'97%',
    	name:'cargoStatus',value:p.get('cargoStatus'),height:50,
    ...
    var txtRemarks = new Ext.form.TextArea({fieldLabel:'备注',anchor:'97%',
    	name:'remarks',value:p.get('remarks'),height:50,
    	enableKeyEvents:true,tabIndex:22
    });
	
  //发货单位-发货方
    var txtShipperName = new Ext.form.TextField({fieldLabel:'发货单位',anchor:'97%',
    ...
    //发货人-发货方
    var txtShipperContact = new Ext.form.TextField({fieldLabel:'发货人',anchor:'97%',
    	name:'shipperContact',value:p.get('shipperContact'),
    	enableKeyEvents:true,tabIndex:24
    });
   //座机电话-发货方
    var txtShipperTel = new Ext.form.TextField({fieldLabel:'座机电话',anchor:'97%',
    ...
    //手机号码-发货方
    var txtShipperMobile = new Ext.form.TextField({fieldLabel:'手机号码',anchor:'97%',
    	name:'shipperMobile',value:p.get('shipperMobile'),
    	enableKeyEvents:true,tabIndex:26
    });
    
   //提货地址
	var txtLoadAddress = new Ext.form.TextArea({fieldLabel:'提货地址',anchor:'99%',
		name:'loadAddress',value:p.get('loadAddress'),height:50,
	...
	//收货方-收货地址
    var txtDeliveryAddress = new Ext.form.TextArea({fieldLabel:'收货地址',anchor:'95%',
    	name:'deliveryAddress',value:p.get('deliveryAddress'),height:50,
    ...
    var cboPateName = new Ext.form.ComboBox({fieldLabel:'付款方式',anchor:'95%',
		...
		typeAhead:true,mode:'remote',triggerAction:'all',selectOnFocus:true,forceSeletion:true,editable:false,
		listeners:{scope:this,
			select:function(c,r,i){
	...
	var cboTransportWay = new Ext.form.ComboBox({fieldLabel:'运输方式',anchor:'95%',
		...
		valueField:'transTypeName',enableKeyEvents:true,editable:false,
  ...
  //代客装卸-是：否
    ...
    var frm = new Ext.form.FormPanel({
    	title:'',layout:'fit',frame:false,iconCls:'list-items',autoScroll:true,padding:5,
    	items:[
    	  {title:'单票信息',xtype:'fieldset',checkboxToggle:false,autoWidth:true,autoHeight:true,layout:'column',labelAlign:'right',items:[
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtConsNo,txtConsNoHandler,cboCustName]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboGrouName,dtConsDate,cboConsBelong]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboSalesRepName,txtStartStation,cboCustContact]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboConsOperatorName,txtEndStation,txtCustMobile]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboTransportWay]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtRemarks]}
    	  ]},
    	  {layout:'column',border:false,items:[
         	 {title:'发货方',xtype:'fieldset',columnWidth:.5,layout:'column',bodyStyle:'padding:5px; 5px; 5px; 0px;',labelAlign:'right',items:[
             {layout:'form',columnWidth:.3,border:false,labelWidth:58,items:[txtShipperName,txtShipperTel]},
       	     {layout:'form',columnWidth:.3,border:false,labelWidth:58,items:[txtShipperContact,txtShipperMobile]},
       	     {layout:'form',columnWidth:.4,border:false,labelWidth:58,items:[txtLoadAddress]}
             ]},
             {title:'收货方',xtype:'fieldset',columnWidth:.5,layout:'column',bodyStyle:'padding:5px; 5px; 5px; 0px;',labelAlign:'right',items:[
        	 {layout:'form',columnWidth:.3,border:false,labelWidth:58,items:[cboConsigneeName,txtConsigneeTel]},
       	     {layout:'form',columnWidth:.3,border:false,labelWidth:58,items:[txtConsigneeContact,txtConsigneeMobile]},
       	     {layout:'form',columnWidth:.4,border:false,labelWidth:58,items:[txtDeliveryAddress]}
       	     ]}
    	  ]},
    	  {title:'运费信息',xtype:'fieldset',checkboxToggle:false,autoWidth:true,autoHeight:true,layout:'column',labelAlign:'right',items:[
       	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtExpenseTotal2]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtExpenseTotal]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtExpenseTotal3]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboArrivePayName]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:70,items:[cboConsigneePateName]},
    	     
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboPateName]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtExpenseXff]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtExpenseDff]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtExpenseHdf]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtExpenseYjf]}
    	  ]}
    	]
    });
    //承运商-物流商
	...
	var txtMotorcadeFax = new Ext.form.TextField({fieldLabel:'传真',enableKeyEvents:true,
		name:'motorcadeFax',value:p.get('motorcadeFax'),tabIndex:4,anchor:'95%',
		listeners:{scope:this,
		 	keydown:function(f,e){
				if(e.getKey()==e.ENTER){
	...
	//第一承运商-省份
	var cboMotorcadeProvince = new Ext.form.ComboBox({fieldLabel:'省份',tabIndex:16,
		name:'motorcadeProvince',value:p.get('motorcadeProvince'),enableKeyEvents:true,
      	store:HTStore.getPROVINCE_S(),displayField:'placName',valueField:'placName',
      	typeAhead:true,mode:'remote',triggerAction:'all',selectOnFocus:true,anchor:'95%',
      	listeners:{scope:this,
      		select:function(c,r,v){
	     		p.set('motorcadeProvinceId',r.get('id'));
	     	 	var bp = {_A:'PLAC_Q',_mt:'xml',placType:2,placProvinceId:r.get('id')};
	     	 	cboMotorcadeCity.store.baseParams= bp;
	     	 	cboMotorcadeCity.store.reload();
	      	},
	      	keydown:function(f,e){
				if(e.getKey()==e.ENTER){
					cboMotorcadeCity.focus();
				} 
			}
      	 }
	});
	//第一承运商-城市
	var cboMotorcadeCity = new Ext.form.ComboBox({fieldLabel:'市/县',tabIndex:17,name:'motorcadeCity',
    	value:p.get('motorcadeCity'),enableKeyEvents:true,
     	store:HTStore.getCITY_S(p.get('motorcadeProvinceId')),xtype:'combo',displayField:'placName',valueField:'placName',
     	typeAhead:true,mode: 'remote',triggerAction: 'all',selectOnFocus:true,anchor:'95%',
     	listeners:{
     		select:function(c,r,v){
	     		p.set('motorcadeCityId',r.get('id'));
	     	},
	     	keydown:function(f,e){
				if(e.getKey()==e.ENTER){
	...
	//物流商-驾驶员
	var cboDriver = new Ext.form.TextField({fieldLabel:C_DRIVER,name:'driverName',value:p.get('driverName'),anchor:'95%'});
	...
	//物流商-车牌号
	var cboVehicle = new Ext.form.TextField({fieldLabel:C_VEHICLE_NO,name:'vehicleNo',value:p.get('vehicleNo'),anchor:'95%'});
	...
	//第一承运商-到货日期
	var dtEndDate = new Ext.form.DateField({fieldLabel:'到货日期',
		name:'endDate',value:p.get('endDate'),tabIndex:30,format:DATEF,anchor:'95%'});
	//第一承运商-地址
	var txtMotorcadeAddress = new Ext.form.TextField({fieldLabel:'地址',enableKeyEvents:true,
		name:'motorcadeAddress',value:p.get('motorcadeAddress'),tabIndex:18,anchor:'95%'
	});
	//物流商-付款方式
	var cboPayWay = new Ext.form.ComboBox({fieldLabel:'付款方式',enableKeyEvents:true,
		...
		typeAhead:true,mode:'remote',triggerAction:'all',selectOnFocus:true,anchor:'95%',
		disabled:p.get('expeSubmitStatus2')=='1',editable:false,
		listeners:{scope:this,
			select:function(c,r,i){
	...
	var cboMotorcade3Province = new Ext.form.ComboBox({fieldLabel:'省份',tabIndex:26,
		name:'motorcade3Province',value:p.get('motorcade3Province'),enableKeyEvents:true,
      	...
      	listeners:{scope:this,
      		select:function(c,r,v){
	     	 	p.set('motorcade3ProvinceId',r.get('id'));
	     	 	var bp = {_A:'PLAC_Q',_mt:'xml',placType:2,placProvinceId:r.get('id')};
	...
	var vendorPanel = new Ext.Panel({title:'承运商信息',layout:'fit',iconCls:'',items:[
	      {title:'',layout:'column',layoutConfig:{columns:5},border:false,frame:false,height:140,padding:5,
	    	  items:[
		      {columnWidth:.2,layout:'form',border:false,labelWidth:58,labelAlign:'right',items:[cboMotorcadeName,cboMotorcadeProvince,cboPayWay,motorcadeExpense2]},
		 	  {columnWidth:.2,layout:'form',border:false,labelWidth:58,labelAlign:'right',items:[txtMotorcadeContact,cboMotorcadeCity,motorcadeExpenseXff,motorcadeExpenseHkf]},
		 	  {columnWidth:.2,layout:'form',border:false,labelWidth:58,labelAlign:'right',items:[txtMotorcadeTel,cboDriver,motorcadeExpenseDff,dtStartDate]},
		 	  {columnWidth:.2,layout:'form',border:false,labelWidth:58,labelAlign:'right',items:[txtMotorcadeFax,cboVehicle,motorcadeExpenseHdf,dtEndDate]},
		 	  {columnWidth:.2,layout:'form',border:false,labelWidth:58,labelAlign:'right',items:[txtMotorcadeNo,txtMotorcadeAddress,motorcadeExpenseYjf]}
	        ]}
	     ]});	
	...
	var exportTemplate = {text:"模板输出",iconCls:'print',
		disabled:NR(M1_CONS+F_O)||p.get('rowAction')=='N',
		menu:{items:[exportConsigneeTemplate,exportReceiptTemplate]}};
	var borderPanel=new Ext.Panel({title:'托运单信息',
		layout:'border',
		items:[{region:'north',autoScroll:true,layout:'fit',height:320,items:[frm]},
			{region:'center',layout:'fit',autoScroll:true,items:[grid]}]
	});
	Fos.TConsTab.superclass.constructor.call(this,{layout:'fit',
		...
		items:[{activeTab:0,xtype:'tabpanel',items:[borderPanel,vendorPanel]}],
		bbar:[getMB(p)]
	});
};
	...
	var panel = new Ext.Panel({plain:true,height:340,layout:'column',title:'综合查询',id:'T_CONS_LOOK_1',
		padding:5,items:[
	    	...
	    	items:[
				...
				{fieldLabel:'承运商',name:'motorcadeName',
