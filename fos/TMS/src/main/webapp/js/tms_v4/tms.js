...
Fos.TConsignGrid = function() {
	var bp={_A:'TCON_Q',_mt:'xml',versionFlag:3};
	...
	{header:boldFont('货物件数'),dataIndex:'packages',renderer:function(v,m,r){m.css='green-b';if(v==''){return 0;}else{return v;}}},
	{header:boldFont('货物毛重'),dataIndex:'grossWeight',renderer:function(v,m,r){m.css='green-b';if(v==''){return 0;}else{return v;}}},
	{header:boldFont('货物体积'),dataIndex:'measurement',renderer:function(v,m,r){m.css='green-b';if(v==''){return 0;}else{return v;}}},
...
Fos.TConsTab = function(p,listStore){
    ...
    var cm=new Ext.grid.ColumnModel({columns:[new Ext.grid.RowNumberer(),sm,
        ...
        {header:'保险费(承运人)',dataIndex:'premiumExpenseProvider',width:120,
              ...
              editor:new Ext.form.NumberField({decimalPrecision:4,allowBlank:false,emptyText:'',invalidText:''}),hidden:true},
         cboConsigneeName,
	  	 {header:'收货人',dataIndex:'consigneeContact',editor:new Ext.form.TextField()},
	  	 {header:'电话',dataIndex:'consigneeTel',editor:new Ext.form.TextField()},
	  	 {header:'收货地址',dataIndex:'deliveryAddress',editor:new Ext.form.TextField()},
	...
	var cargGrid = new Ext.grid.EditorGridPanel({title:'货物列表',iconCls:'gen',clicksToEdit:1,sm:sm,cm:cm,store:store,
	...
	//手机-委托方
	var txtCustMobile = new Ext.form.TextField({fieldLabel:'电话',anchor:'95%',
  ...
  //发货单位-发货方
   ...
   //提货地址
	var txtLoadAddress = new Ext.form.TextField({fieldLabel:'提货地址',anchor:'97.5%',
  ...
  //代客装卸-是：否
    ...
    var frm = new Ext.form.FormPanel({
    	title:'',frame:false,iconCls:'list-items',bodyStyle:'padding:5px;',
    	items:[
    	  {title:'单票信息',xtype:'fieldset',checkboxToggle:false,layout:'column',labelAlign:'right',items:[
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtConsNo,dtConsDate]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[txtConsNoHandler,cboCustName]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboSalesRepName,cboCustContact]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboConsOperatorName,txtCustMobile]},
    	     {layout:'form',columnWidth:.2,border:false,labelWidth:58,items:[cboGrouName,txtStartStation]}
    	  ]},
    	  {layout:'column',border:false,items:[
    	  	{title:'发货方',xtype:'fieldset',columnWidth:.6,layout:'column',labelAlign:'right',bodyStyle:'padding:5px;',items:[
    	  		{layout:'form',columnWidth:.33,border:false,labelWidth:58,items:[txtShipperName]},
	       	    {layout:'form',columnWidth:.33,border:false,labelWidth:58,items:[txtShipperContact]},
	       	    {layout:'form',columnWidth:.34,border:false,labelWidth:58,items:[txtShipperTel]},
	       	    {layout:'form',columnWidth:.33,border:false,labelWidth:58,items:[txtShipperMobile]},
	       	    {layout:'form',columnWidth:.67,border:false,labelWidth:58,items:[txtLoadAddress]}
             ]},
             {layout:'form',columnWidth:.4,border:false,labelWidth:58,labelAlign:'right',items:[txtRemarks]}
    	  ]}
    	]
    });
	...
	Fos.TConsTab.superclass.constructor.call(this,{layout:'border',height:800,
		...
		items:[{region:'north',autoScroll:true,layout:'fit',height:200,items:[frm]},
