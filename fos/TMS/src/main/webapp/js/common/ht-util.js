var HTUtil= {
	...
	*/
	...
	RUA : function(store,a,rt){
		...
		for(var j=0;j<a.length;j++){	
			for(var i=0;i<sa.length;i++){
				...
				if(((sa[i].get('rowAction') == 'M' || a[j].get('rowAction') == '') && sa[i].get('id') == a[j].get('id'))
						|| (sa[i].get('rowAction') == 'N' && sa[i].get('uuid')==a[j].get('uuid'))){				
	...
	POST_SMS :function(action,u,n,pwd,k,m,c,consId,consNo,bizType,consNoHandler){
		Ext.Ajax.request({scope:this,url:SERVICE_URL,method:'POST',
			params:{_A:action,smsUrl:u,smsUser:n,smsPassword:pwd,smsKey:k,
			mobile:m,content:c,consId:consId,consNo:consNo,bizType:bizType,consNoHandler:consNoHandler},
			success: function(r){
