...
var getSysPanel = function(){
	...
	var items=[];
	if(HR(M1_SYS+P_OFFICE))
		items[items.length]=FosMenu(panel,'办事处','OFFICE',function(){return new Fos.OfficeTab();});
	if(HR(M1_SYS+P_GROU)||ROLE_ID==0)
		items[items.length]=FosMenu(panel,'公司部门','T_GROU',function(){return new Fos.GroupTab();});
	if(HR(M1_SYS+P_ROLE)||ROLE_ID==0)
		items[items.length]=FosMenu(panel,'角色权限','S_SET_FUNCTION',function(){return new Fos.RoleTab();});
	if(HR(M1_SYS+P_USER)||ROLE_ID==0)
		items[items.length]=FosMenu(panel,'用户管理','T_USER',function(){return new Fos.UserTab();});
	if(HR(M1_SYS+P_TEMP))
		items[items.length]=FosMenu(panel,'模版管理','T_TEMP',function(){return new Fos.TempTab();});
	if(HR(M1_SYS+P_COCO))
		items[items.length]=FosMenu(panel,'系统参数','T_COCO',function(){return new Fos.CocoTab();});
	if(HR(M1_SYS+P_ACLO))
		items[items.length]=FosMenu(panel,'操作日志','T_ACLO',function(){return new Fos.AcloGrid();});
	
	var genPanel = new Ext.Panel({title:C_SYSTEM_MGT,collapsible:true,layout:'fit',iconCls:'folder_go',
	...
	createWindow('SYS',C_SYSTEM_MGT,contPanel);//C_SYSTEM_MGT
};
...
Fos.GroupTab = function() {
	...
	var cm=new Ext.grid.ColumnModel({columns:[
		...
		active],defaults:{sortable:false,width:130}});
	
	Fos.GroupTab.superclass.constructor.call(this,{id:'T_GROU',title:C_GROU,closable:true,iconCls:'gen',
		plugins:[active],clicksToEdit:1,store:store,sm:sm,cm:cm,
		tbar:[{itemId:'TB_N',text:C_ADD,disabled:MR(M1_SYS+P_GROU),iconCls:'add',scope:this,handler:this.add}, '-', 		
			{itemId:'TB_R',text:C_REMOVE,disabled:MR(M1_SYS+P_GROU),iconCls:'remove',scope:this,handler:this.del},'-', 
			{itemId:'TB_S',text:C_SAVE,disabled:MR(M1_SYS+P_GROU),iconCls:'save',scope:this,handler:this.save},'-']
		});
};
Ext.extend(Fos.GroupTab, Ext.grid.EditorGridPanel);

//办事处
Fos.OfficeTab = function() {
	...
	var cm=new Ext.grid.ColumnModel({columns:[
	      new Ext.grid.RowNumberer(),
	      sm,
          {header:C_NAME,dataIndex:'siteName',editor:new Ext.form.TextField()}
	...
	Fos.OfficeTab.superclass.constructor.call(this,{
    id:'G_SITE',iconCls:'gen',title:'办事处',header:false,clicksToEdit:1,closable:true,
    store: store,sm:sm,cm:cm,loadMask:true,
	tbar:[{
		text:C_ADD,disabled:NR(M1_SYS+P_OFFICE),iconCls:'add',scope:this,handler:this.add},'-',
        {text:C_REMOVE,disabled:NR(M1_SYS+P_OFFICE),iconCls:'remove',scope:this,handler:this.del}, '-', 
        {text:C_SAVE,disabled:NR(M1_SYS+P_OFFICE),iconCls:'save',scope:this,handler:this.save},'-'
        ]
    });
};
Ext.extend(Fos.OfficeTab, Ext.grid.EditorGridPanel);
...
Fos.RoleTab = function() {
	...
	var cm=new Ext.grid.ColumnModel({columns:[
		...
		{header:C_DESC,dataIndex:'roleDesc',editor:new Ext.form.TextField(),hidden:true},
		active],defaults:{sortable:false,width:130}});
	this.add=function(){
	...
	Fos.RoleTab.superclass.constructor.call(this,{id:'T_ROLE',title:S_SET_FUNCTION,closable:true,iconCls:'gen',
		plugins:active,clicksToEdit:1,store:store,sm:sm,cm:cm,
		tbar:[{itemId:'TB_N',text:C_ADD,disabled:MR(M1_SYS+P_ROLE),iconCls:'add',scope:this,handler:this.add},'-',
		{itemId:'TB_R',text:C_REMOVE,disabled:MR(M1_SYS+P_ROLE),iconCls:'remove',handler:this.del},'-',
		{itemId:'TB_S',text:C_SAVE,disabled:MR(M1_SYS+P_ROLE),iconCls:'save',scope:this,handler:this.save},'-',
		{itemId:'TB_P',text:C_PERMISSION_SET,disabled:MR(M1_SYS+P_ROLE),iconCls:'key',scope:this,handler:this.setPermission},'-']
	});
};
...
Fos.UserTab = function() {
	...
	//可查看所有单票
	var va = new Ext.grid.CheckColumn({header:C_VIEW_ALL_CONS,dataIndex:'userAllViewFlag',width:120});
	//可编辑所有单票
	var ea = new Ext.grid.CheckColumn({header:C_EDIT_ALL_CONS,dataIndex:'userAllEditFlag',width:120});
	//是否是外部帐号
	var utf = new Ext.grid.CheckColumn({header:'外部帐号',dataIndex:'userTypeFlag',width:100});
	//帐号所属外部客户帐号
	var outerCustNameCn={header:'外部客户名称',width:200,dataIndex:"custNameCn",align:'left',
            ...
            listeners:{scope:this,
            	select:function(c,r,i){
					...
					b.set('custNameCn',r.get('custNameCn'));
				},
	...
	var cm=new Ext.grid.ColumnModel({columns:[
		...
		{header:C_SYS_USER_NAME,dataIndex:'userLoginName',editor:new Ext.form.TextField()},
		{header:C_DEFAULT_GROU,dataIndex:'grouName',
		...
		{header:'所属办事处',dataIndex:'siteName',
			editor:new Ext.form.ComboBox({displayField:'siteName',valueField:'siteName',triggerAction: 'all',
				allowBlank:false,mode:'remote',selectOnFocus:true,listClass:'x-combo-list-small',
        ...
        {header:C_DEFAULT_ROLE,dataIndex:'roleName',hidden:true,
	...
	this.add=function(){
		...
		userOperatorFlag:'0',userTypeFlag:'0',custNameCn:'',active:'1',version:'0',rowAction:'N'});
		this.stopEditing();
    	store.insert(0,r);
    	this.startEditing(0,1);
	};
	...
	this.save = function(){
		...
		var a=store.getRange();
		for(var i=0;i<a.length;i++){
			if(a[i].get('userLoginName')==''){
				XMG.alert(SYS,'系统登录名不能为空！');
				return;
			}
			if(a[i].get('grouName')==''){
				XMG.alert(SYS,'所属部门不能为空！');
				return;
			}
			if(a[i].get('userTypeFlag')=='1'){
				if(a[i].get('custNameCn')==''){
					XMG.alert(SYS,'外部客户名称不能为空！');
					return ;
				}
			}
		}
	...
	this.reset = function(){
		...
		if(b){
			Ext.Ajax.request({url:SERVICE_URL,method:'POST',
				params:{_A:'USER_UA',id:b.get('id'),newPassword:'123456',newPassword2:'123456'},
    ...
    Fos.UserTab.superclass.constructor.call(this,{id:'T_USER',title:C_USER_MGT,iconCls:'gen',closable:true,    	
		clicksToEdit:2,border:false,store:store,sm:sm,cm:cm,
		plugins:[sales,operator,va,ea,active,utf],
		tbar:[{itemId:'TB_N',text:C_ADD,disabled:MR(M1_SYS+P_USER),iconCls:'add',scope:this,handler:this.add}, '-', 		
		{itemId:'TB_R',text:C_REMOVE,disabled:MR(M1_SYS+P_USER),iconCls:'remove',handler:this.del},	'-', 		
		{itemId:'TB_S',text:C_SAVE,disabled:MR(M1_SYS+P_USER),iconCls:'save',scope:this,handler:this.save},'-', 	
		{itemId:'TB_P',text:C_RESET_PASS,disabled:MR(M1_SYS+P_USER),iconCls:'save',scope:this,handler:this.reset},'-',
		{itemId:'TB_E',text:S_USER_SET_ROLE,disabled:MR(M1_SYS+P_USER),iconCls:'save',scope:this,handler:this.showRole},'-']
	});
};
...
Fos.UserRoleWin = function(user) {
    ...
    var grid = new  Ext.grid.GridPanel({border:false,store:HTStore.getROLE_S(),sm:sm,
    	cm:new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),sm,{header:C_ROLE_NAME,dataIndex:'roleName',width:100}])
    	});
...
Fos.TempTab = function() {
	...
	var cm=new Ext.grid.ColumnModel({columns:[
		...
		{header:C_TEMP_DESC,dataIndex:'tempDesc',width:200,editor:new Ext.form.TextField()},
		...
		{header:C_TEMP_TYPE,dataIndex:'tetyName',width:200,
    ...
    Fos.TempTab.superclass.constructor.call(this,{id:'G_TEMP',title:C_TEMP_MGT,iconCls:'gen',closable:true,    	
    	...
    	tbar:[{itemId:'TB_N',text:C_ADD,disabled:NR(M1_SYS+P_TEMP),iconCls:'add',scope:this,handler:this.add},'-',
            {itemId:'TB_R',text:C_REMOVE,disabled:NR(M1_SYS+P_TEMP),iconCls:'remove',scope:this,handler:this.del},
            {itemId:'TB_S',text:C_SAVE,disabled:NR(M1_SYS+P_TEMP),iconCls:'save',scope:this,handler:this.save},
            {itemId:'TB_D',text:C_TEMP_DOWNLOAD,disabled:NR(M1_SYS+P_TEMP),iconCls:'down',scope:this,handler:this.download},
            {itemId:'TB_U',text:C_TEMP_UPLOAD,disabled:NR(M1_SYS+P_TEMP),iconCls:'up',scope:this,handler:this.upload},'-'
            ]
        });
};
...
Fos.CocoTab = function() {
	...
	var cm=new Ext.grid.ColumnModel({columns:[
     	new Ext.grid.RowNumberer(),
     	{header:C_COCO_CODE,dataIndex:'cocoCode',width:300},
 		{header:C_COCO_NAME,dataIndex: 'cocoName',width:200},
 		{header:C_COCO_VALUE,dataIndex:'cocoValue',width:200,
 		...
 		{header:C_COCO_GROU,dataIndex:'cocoGroup',width:200}
 	],
 	editors: {
		...
		'port':new Ext.grid.GridEditor(new Ext.form.ComboBox({typeAhead:true,mode:'remote',triggerAction:'all',selectOnFocus:true,
			tpl:portTpl,itemSelector:'div.list-item',listWidth:C_LW,store:HTStore.getPS(),displayField:'portNameEn',valueField:'portId',
			listeners:{scope:this,select:function(c,r,i){
        	var b =sm.getSelected();
		...
		'char':new Ext.grid.GridEditor(new Ext.form.ComboBox({typeAhead:true,mode:'remote',triggerAction:'all',selectOnFocus:true,
			tpl:charTpl,itemSelector:'div.list-item',listWidth:C_LW,store:HTStore.getCHAR_S(),displayField:'charCode',valueField:'charId',
			listeners:{scope:this,select:function(c,r,i){
        		var b =sm.getSelected();
    	...
    	'grou':new Ext.grid.GridEditor(new Ext.form.ComboBox({typeAhead:true,mode:'remote',triggerAction:'all',selectOnFocus:true,
			store:HTStore.getGROU_S(),displayField:'grouName',valueField:'grouId',
			listeners:{scope:this,select:function(c,r,i){
        		var b =sm.getSelected();
    	...
    	'fdoc':new Ext.grid.GridEditor(new Ext.form.ComboBox({typeAhead:true,
			...
			store:HTStore.getDOTY_S(),displayField:'dotyName',valueField:'dotyId',
			listeners:{scope:this,select:function(c,r,i){
        		var b =sm.getSelected();
	...
	getCellEditor:function(colIndex, rowIndex) {
		...
		if (field == 'cocoValue') {
			...
			else if(rec.get('cocoValueType')==4){
    ...
    tbar:[{itemId:'TB_S',text:C_SAVE,disabled:NR(M1_SYS+P_COCO),iconCls:'save',scope:this,handler:this.save},'-']
	});
};
...
Fos.MetoTab = function(p){
	...
	var cm=new Ext.grid.ColumnModel({columns:[sm,
		{header:C_NAME,dataIndex:'metoName',width:120},
		{header:C_DESC,dataIndex:'metoDesc',width:80},
...
Fos.SubscriberWin = function(topic) {
	...
	Fos.SubscriberWin.superclass.constructor.call(this,{iconCls:'key',title:C_SUBSCRIBER_LIST,
       ...
       tbar:[
          ...
          {itemId:'TB_S',text:C_SAVE,disabled:NR(M1_SYS+P_ROLE),iconCls:'save',scope:this,handler:this.save},'-']
      });
};
...
Fos.AcloGrid = function() {
	var store = new Ext.data.Store({url:SERVICE_URL,baseParams:{_A:'ACLO_X',_mt:'xml'},
		reader:new Ext.data.XmlReader({totalProperty:'rowCount',record:'PActionLog',id:'id'},PActionLog),
		remoteSort:true,sortInfo:{field:'id', direction:'desc'}});	
	
	store.load({params:{start:0,limit:25}});
	var c1={header:C_ACT_USER,dataIndex:"acloUserName"};
	var c2={header:C_ACT_TIME,dataIndex:"createTime",width:180};
	...
	var c6={header:C_ACT_IP,width:150,dataIndex:"acloIp"};
	var sm=new Ext.grid.CheckboxSelectionModel({singleSelect:false});
	var cm=new Ext.grid.ColumnModel({columns:[new Ext.grid.RowNumberer(),sm,c1,c2,c3,c4,c5,c6],defaults:{sortable:false,width:100}});
	cm.defaultSortable = true;
	cm.defaultWidth=100;
	this.search = function(){var w=new Fos.AcloLW(store);w.show();};	
	Fos.AcloGrid.superclass.constructor.call(this, {id:'G_ACLO',title:C_ACT_LOG,iconCls:'gen',
		border:true,autoScroll:true,stripeRows:true,store:store,sm:sm,cm:cm,loadMask:true,closable:true,
	    bbar:PTB(store,C_PS100),
	    tbar:[{itemId:'TB_F',text:C_SEARCH,iconCls:'search',handler:this.search},'-',
...
Fos.AcloLW = function(store) {
	this.reload=function(){
		var a=[];
		var acloUserName=this.find('name','acloUserName')[0].getValue();
		if(acloUserName) a[a.length]=new QParam({key:'acloUserName',value:acloUserName,op:EQ});
		var acloTno=this.find('name','acloTno')[0].getValue();
		if(acloTno) a[a.length]=new QParam({key:'acloTno',value:acloTno,op:EQ});
		var createTime=this.find('name','createTime')[0].getValue();
		var createTime2=this.find('name','createTime2')[0].getValue();
		if(createTime && createTime2){
     		a[a.length]=new QParam({key:'createTime',value:createTime.format('Y-m-d H:i:s'),op:GE});
     		a[a.length]=new QParam({key:'createTime',value:createTime2.format('Y-m-d H:i:s'),op:LE});
     	}
     	else if(createTime) a[a.length]=new QParam({key:'createTime',value:createTime,op:EQ});
		store.baseParams={_A:'ACLO_X',_mt:'xml',xml:HTUtil.HTX(HTUtil.QTX(a))};
	...
	var frm = new Ext.form.FormPanel({labelWidth:100,bodyStyle:'padding:5px',
    	items: [{fieldLabel:C_ACT_USER,name:'acloUserName',store:HTStore.getUSER_S(),
    		xtype:'combo',displayField:'userLoginName',valueField:'userName',
			typeAhead: true,mode: 'remote',triggerAction: 'all',selectOnFocus:true,anchor:'95%'},
	...
	Fos.AcloLW.superclass.constructor.call(this, {title:'操作日志查询',iconCls:'search',modal:true,width:400,minWidth:300,
...
Fos.FileUploadWin = function(title,label) {
	var fp = new Ext.form.FormPanel({
        id:'F_UP',fileUpload:true,title:title,header:false,autoHeight:true,labelWidth:110,
        items: [{
            xtype:'fileuploadfield',emptyText:label,fieldLabel:'请选择模版文件',
            name:'tempFile',anchor:'95%',frame:true,allowBlank:false,msgTarget:'side'
        }]
    });
    Fos.FileUploadWin.superclass.constructor.call(this, {
    	title:title,iconCls:'up',plain:false,modal:true,width:420,minWidth:300,
