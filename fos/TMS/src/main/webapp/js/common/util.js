...
var reportUrl='http://www.hitisoft.com:8080';

REPORT_URL=reportUrl+'/TmsReport/preview?__locale=zh_CN&__navigationbar=false&__showtitle=false';
...
var openCons=function(n){
	store = new Ext.data.Store({url: SERVICE_URL+'?_A='+'TCON_Q',
		reader:new Ext.data.XmlReader({record:'TConsign'},TConsign)});
	store.load({params:{consNoHandler:n,typeKey:'1'},
		callback:function(re,o,s){
			if(s&&re.length>0) {
				var p=re[0];
				var id='CONS_'+p.get("id");
				var title=C_CONSIGN+'_'+p.get('consNoHandler');
				var item=new Fos.TConsTab(p);
				var modal=true;
				var width='';
				var height='';
				...
				createWindow(id,title,item,modal,width,height);
			}
	}});};
...
var numRenderColor  = function(v,m,r){v = parseFloat(v);v=v.toFixed(2);if(v=='NaN') v='0.00';m.css=(v>0?'green-b':'red-b');return v;};
...
var rateRender = function(v){v=parseFloat(v);v=v.toFixed(2);if(v=='NaN') v='0.00';return v;};
...
//字体加粗
var boldFont=function(s){return '<font>'+s+'</font>';};
//使用图片
var iconFont=function(name){
	return '<img src="./././images/'+name+'.png"/>';
};

var ffaa66='background:#ffaa66;';
var ffdd66='background:#ffdd66;';
var ffaa88='background:#ffaa88;';
...
var C_CT={header:"创建时间",width:130,align:'right',renderer:formatDateTime,dataIndex:"createTime"};
var C_MT={header:"修改时间",width:130,align:'right',renderer:formatDateTime,dataIndex:"modifyTime"};
var getMB = function(p){return [{xtype:'tbtext',text:C_CREATE_BY_C+HTStore.getUSER(p.get('createBy'))},'-',
...
var loadCustomer=function(f,e,custType,bizType,simpleFlag){
	if(e.getKey()!=e.ENTER){	
		...
		if(q.length>1 && !f.isExpanded()){
			var a=[];
			a[0]=new QParam({key:'custCode',value:q+'%',op:LI});
			if(custType!='') a[a.length]=new QParam({key:custType,value:'1',op:EQ});
			if(bizType!=''){
				var k = '';
...
var tmsConsinessList=function(f,e){
	if(e.getKey()!=e.ENTER){
		var q=f.getRawValue();
		if(q.length>0 && !f.isExpanded()){
	   		Ext.Ajax.request({url:SERVICE_URL,method:'POST',params:{_A:'TMS_CONSIGNEE_Q',_mt:'xml',consigneeName:q},
				success: function(r,o){
					f.store.loadData(r.responseXML,false);
					f.expand();
				}
