Fos.CargoOutReportTab = function(t,b,n){
	...
	var custId='';
	var LUCustName=new Fos.CustomerLookup({fieldLabel:'收货人',
		...
		listeners:{scope:this,
		select:function(c,r,i){
			LUCustName.setValue(r.get('custNameCn'));
			custId=r.get('id');
		},
	...
	this.report=function(n){
		...
		if(storageClass.getValue()){
			url +=	'&storageClass='+storageClass.getValue();
		}
...
var frm=new Ext.form.FormPanel({title:'综合过滤条件',layout:'column',name:'sf',xtype:'form',layoutConfig:{columns:3},
	...
	items:[
             {columnWidth:.25,layout:'form',labelWidth:80,labelAlign:'right',border:false,
            	  items:[cargoOwenrName,ccboBlockName,LUCustName]},
             {columnWidth:.25,layout:'form',labelWidth:80,labelAlign:'right',border:false,
            	   items:[storageClass,ccboAreaName]},
             {columnWidth:.25,layout:'form',labelWidth:80,labelAlign:'right',border:false,
                	items:[cargoType,ccboWarehouseName]},
 		     {columnWidth:.25,layout:'form',labelWidth:80,labelAlign:'right',border:false,
                	items:[cargoName,status]}
 		]});
