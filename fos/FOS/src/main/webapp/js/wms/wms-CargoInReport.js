Fos.CargoInReportTab = function(t,b){
	...
	var report1=function(v){
		 
		var url = REPORT_URL;
		
			url += '&__report=reports/wms_CargoInReportBSD.rptdesign&compCode='+sessionStorage.getItem("COMP_CODE");
	...
	var btnSearch1=new Ext.Button({text:"生成报表新",iconCls:'stats',hidden:NR(M1_WMS+WM_NOTEIN+WMI_IN+WF_SEARCH),
		scope:this,handler:function(){report1('S');}});
	...
	var btnExpExcel1=new Ext.Button({text:"输出新",iconCls:'print',hidden:NR(M1_WMS+WM_NOTEIN+WMI_IN+WF_EXPORT),
		scope:this,handler:function(){report1('E');}});
	
	var btnExport=new Ext.Button({text:'查看报表',iconCls:'stats',scope:this,
		menu: {items: [btnSearch,btnSearch1]
    }});
	
	var btnExport1=new Ext.Button({text:C_EXPORT,iconCls:'print',scope:this,
		menu: {items: [btnExpExcel,btnExpExcel1]
    }});
	...
	Fos.CargoInReportTab.superclass.constructor.call(this, {    
    	id:'CargoInReport'+t,iconCls:'stats',title:b,deferredRender:false,closable:true,autoScroll:true,
     	tbar:[{xtype:'tbtext',text:'日  期:'},storageDate,{text:'来源类型:'},storageClass,'-',fromDate,'-',toDate,'-',
     	     t==1?btnExport:btnSearch,t==1?btnExport1:btnExpExcel],
		items:[frm1,doc]});
};
...
Fos.ShowPlacedAndPickedCargoTab = function(){
    ...
    var txtStorageNo=new Ext.form.TextField({fieldLabel:'入库单号',anchor:'95%'});
  ...
  //仓库
    ...
    var re4={
		...
		rowselect: function (sm,row,record){
			if(record){
				store.baseParams={_A:'WPLACED_CARGO_Q',_mt:'xml',storageNoteCargoId:record.get('id')};
    ...
    var grid=new Ext.grid.GridPanel({iconCls:'gen',title:'入库货物明细',
    	layout:'fit',height:200,region:'center',autoscoll:'true',bbar:PTB(store,5),
	...
	this.search=function(){
		...
		if(cargoOwenrName.getValue()){
			a[a.length]=new QParam({key:'cargoOwenrName',value:cargoOwenrName.getValue(),op:op});
		}
		if(txtStorageNo.getValue()){
			a[a.length]=new QParam({key:'storageNoteNo',value:txtStorageNo.getValue(),op:LI});
		}
		if(cargoType.getValue()){
		...
		if(txtEntrustNo.getValue()){
			a[a.length]=new QParam({key:'orderNo',value:txtEntrustNo.getValue(),op:op});
		}
	...
	var frm1=new Ext.form.FormPanel({
		...
		items:[
                 ...
                 {columnWidth:.2,layout:'form',labelWidth:80,labelAlign:'right',border:false,
                	   items:[txtStorageNo,cboWarehouse]},
                 {columnWidth:.2,layout:'form',labelWidth:80,labelAlign:'right',border:false,
	...
	var tab2=new Ext.TabPanel({activeTab:0,region:'center',height:212,autoScroll:true,
		items:[grid,grid1]});
	
	var frm=new Ext.Panel({
		title:'主信息',height:435,
		layout:'column',
