...
Fos.InventorySearchPanel=function(t){
	...
	var refresh=function(){
		...
		if (lkpCargoOwner.getValue())//供应商
		{
			a[a.length]=new QParam({key:'cargoOwnerName',value:lkpCargoOwner.getValue(),op:LE});
		}
		
		if (cargoType.getValue())//产品类别
		{
			a[a.length]=new QParam({key:'categoryName',value:cargoType.getValue(),op:LE});
		}
		
		if (txtOrderNo.getValue())//客户订单号 
		{
			a[a.length]=new QParam({key:'orderNo',value:txtOrderNo.getValue(),op:LE});
		}
		
		if (txtProductNo.getValue())//生产编号
		{
			a[a.length]=new QParam({key:'productNo',value:txtProductNo.getValue(),op:LE});
		}
