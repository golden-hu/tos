...
Fos.BlockLookUp=Ext.extend(Ext.form.ComboBox,{
	...
	onTriggerClick:function(event){
		var win = new Fos.BlockLookWin(this.selectBlock,this.warehouseName,this.areaName,0,this);
		win.show();
	}
});
...
Fos.BlockLookWin=function(fn,warehouseName,areaName,isAll,scope) {
	...
	var radCode=new Ext.form.Radio({checked:true,name:'all',inputValue:'0',boxLabel:'无库存'});
	var radName=new Ext.form.Radio({name:'all',inputValue:'1',boxLabel:'全部'});
	...
	/*
	if(warehouseName!=''){
		ccboWarehouse.setValue(warehouseName);
	};
	
	if(areaName!=''){
		ccboArea.setValue(areaName);
	};*/
