...
cdteStorageDate,ccboStatus]},
			{columnWidth:.5,layout:'form',labelWidth:95,border:false,items:[
ctxtRequirement ]}
			]
	});
    ...
    var atGrid=new Ext.grid.EditorGridPanel({
    	iconCls:'gen',title:'托盘管理',layout:'fit',autoscoll:'true',
    ...
    var CostListGrid=new Ext.grid.EditorGridPanel({
    	iconCls:'gen',title:'计费项目',layout:'fit',autoscoll:'true',
    	...
    	tbar:[{text:C_ADD,iconCls:'add',ref:'../addButton',
    		hidden:(NR(M1_WMS+WM_NOTEIN+WMI_IN+WF_SAVE)),scope:this,handler:addRate},'-',
    	      {text:C_SAVE,iconCls:'save',ref:'../saveButton',hidden:(NR(M1_WMS+WM_NOTEIN+WMI_IN+WF_EDIT)),scope:this,handler:saveRate},'-',    	      
    	      {text:C_REMOVE,iconCls:'remove',ref:'../removeButton',hidden:(NR(M1_WMS+WM_NOTEIN+WMI_IN+WF_DEL)),scope:this,handler:delRate}]
	});
	...
	var btnCheck=new Ext.Button({text:C_COMMIT,iconCls:'check',
		hidden:(NR(M1_WMS+WM_NOTEIN+WMI_IN+WF_COMMIT)),scope:this,handler:this.submit});
	var btnUnCheck=new Ext.Button({text:C_COMMIT_CANCEL,iconCls:'renew',
		hidden:(NR(M1_WMS+WM_NOTEIN+WMI_IN+WF_CANCEL_COMMIT)),scope:this,handler:this.unSubmit});
	 //收货
	...
	var computeCost=new Ext.Button({text:'计费',iconCls:'dollar',
		hidden:(NR(M1_WMS+WM_NOTEIN+WMI_IN+WF_EXPENSE)),scope:this,handler:this.computeCost});
	...
	var tab=new Ext.TabPanel({activeTab:0,region:'center',height:200,
		items:[basicPanel,cargoPanel,atGrid,CostListGrid]});
...
Fos.WNoteCargoWin = function(p,store) {
	...
	var ccboUnitName=new Ext.form.ComboBox({fieldLabel:'EA单位',name:"unitName",value:p.get("unitName"),align:'center',
		      tabIndex:62,disabled:true,itemCls:'required',
	...
	var ccboPUnit=new Ext.form.ComboBox({fieldLabel:'包装单位',name:'pUnit',value:p.get('pUnit'),xtype:'combo',displayField:'unitName',valueField:'unitName',triggerAction:'all',
	...
	var cnumPlanedPackages=new Ext.form.NumberField({fieldLabel:'包装数量',name:'planedPackages',value:p.get('planedPackages'),
