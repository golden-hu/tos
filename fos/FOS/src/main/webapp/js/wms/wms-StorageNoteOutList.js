...
Fos.StorageNoteOutGrid = function(t) {
    ...
    var pallComplete=function(){
    	var p=sm.getSelected();
    	if(p){
    		if(p.get('actionGategory')=='栈板出库'){
    		Ext.Msg.confirm(SYS,"没有货物才能栈板出库！是否确定栈板出库？",function(btn){
	        	if(btn == 'yes') {
    			Ext.Ajax.request({scope:this,url:SERVICE_URL,method:'POST',
					params:{_A:'WSTORAGE_NOTE_O',id:p.get('id'),status:5},
					success:function(r){
						var a=HTUtil.XTRA(r.responseXML,'WStorageNote',WStorageNote);
						HTUtil.RUA(store,a,WStorageNote);
						XMG.alert(SYS,"栈板出库完成！");
					},
					failure:function(r){
						XMG.alert(SYS,M_F+r.responseText);
					}
	        ...
	        this);
    			...
    			XMG.alert(SYS,'类别不是"栈板出库"状态请修改类别！');
    		}
    ...
    var transList=function(){
    	...
    	if(p){
    		if(p.get('status')==5){
    			Ext.Ajax.request({scope:this,url:SERVICE_URL,method:'POST',
    				params:{_A:'WSTORAGE_NOTE_T',id:p.get('id')},companyName:HTStore.getCFG('COMPANY_NAME'),
    				success:function(r){
    ...
    //var btnExp=new Ext.Button({text:'输出拣货单Excel',iconCls:'print',scope:this,handler:this.expExcel,anchor:'95%'});
    ...
    var btnPallComplete=new Ext.Button({text:'栈板出库',iconCls:'check',name:'btnPickedComplete',
    	hidden:NR(M1_WMS+WM_NOTEOUT+WMI_OUT+WF_FINISHED),
    	scope:this,handler:pallComplete});
