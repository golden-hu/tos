...
var txtAtBland=new Ext.form.ComboBox({fieldLabel:'品牌',name:'atBrand',value:p.get('atBrand'),anchor:'95%',
	...
	//客户要求
	var txtCustRequirement=new Ext.form.TextField({fieldLabel:'客户要求',name:'custRequirement',value:p.get('custRequirement'),
			xtype:'textfield',
			tabIndex:8,anchor:'95%'});
		
	//装货地址
	var txtPol=new Ext.form.TextField({fieldLabel:'装货地',name:'pol',value:p.get('pol'),tabIndex:13,xtype:'textfield',anchor:'95%'});
	
	//卸货地址
	var txtPod=new Ext.form.TextField({fieldLabel:'卸货地',name:'pod',value:p.get('pod'),xtype:'textfield',tabIndex:2,anchor:'95%'});
	//装运日期
	//目的地
	var txtDestination=new Ext.form.TextField({fieldLabel:'目的地',name:'destination',value:p.get('destination'),tabIndex:3,xtype:'textfield',anchor:'95%'});
	
	
	var ccboTrans=new Ext.form.ComboBox({fieldLabel:'运输方式',name:'inUnitValue',
		store:WHTStore.IN_UNIT_NAME,
	...
	var txtTruckType=new Ext.form.TextField({fieldLabel:'车型',name:'truckType',value:p.get('truckType'),
		tabIndex:1,anchor:'95%'});
	var txtTruckNumber=new Ext.form.TextField({fieldLabel:'车号',name:'truckNumber',value:p.get('truckNumber'),
		tabIndex:1,anchor:'95%'});
	//付款方式
	var cboPateName=new Ext.form.ComboBox({fieldLabel:'付款方式',tabIndex:7,name:'pateName',value:p.get('pateName'),
	...
	var ccboGaveCargo=new Ext.form.ComboBox({fieldLabel:'交货条款',name:'deliName',value:p.get('deliName'),
		store:HTStore.getPATE_S(),displayField:'pateName',valueField:'pateName',typeAhead: true,mode:'local',
		triggerAction: 'all',selectOnFocus:true,anchor:'95%',
		listeners:{}});
	var txtPayment=new Ext.form.TextField({fieldLabel:'付款描述',name:'pateRemark',value:p.get('pateRemark'),anchor:'95%'});
	
	var txtGaveGargo=new Ext.form.TextField({fieldLabel:'交货描述',name:'deliRemark',value:p.get('deliRemark'),
		anchor:'95%'});
	
	var txtNumber=new Ext.form.TextField({fieldLabel:'总件数',name:'quantity',value:p.get('quantity'),anchor:'95%'});
	
	var txtGW=new Ext.form.TextField({fieldLabel:'总毛重',name:'grossWeight',value:p.get('grossWeight'),anchor:'95%'});
	
	var txtGN=new Ext.form.TextField({fieldLabel:'总净重',name:'netWeight',value:p.get('netWeight'),anchor:'95%'});
	
	var txtVolume=new Ext.form.TextField({fieldLabel:'总体积',name:'volume',value:p.get('volume'),anchor:'95%'});
	
	var txtCost=new Ext.form.TextField({fieldLabel:'总价值',name:'cargoValue',value:p.get('cargoValue'),anchor:'95%'});
...
//不带托盘的出库编辑界面
Fos.StorageNoteOutTab = function(p) {
...
var txtAtBland=new Ext.form.ComboBox({fieldLabel:'品牌',name:'atBrand',value:p.get('atBrand'),anchor:'95%',
	...
	//装货地址
	var txtPol={fieldLabel:'装货地址',name:'loadingaddress',value:p.get('loadingaddress'),tabIndex:13,xtype:'textfield',anchor:'95%'};
	
	//卸货地址
	var txtPod={fieldLabel:'卸货地址',name:'unloadingaddress',value:p.get('unloadingaddress'),xtype:'textfield',tabIndex:2,anchor:'95%'};
	//装运日期
	...
	//目的地
	var txtDestination={fieldLabel:'目的地',name:'destination',value:p.get('destination'),tabIndex:3,xtype:'textfield',anchor:'95%'};
	
	//付款方式
	var cboPateName={fieldLabel:'付款方式',tabIndex:7,name:'pateName',value:p.get('pateName'),store:HTStore.getPATE_S(),
    		xtype:'combo',displayField:'pateName',valueField:'pateName',typeAhead:true,
    		mode: 'remote',triggerAction: 'all',selectOnFocus:true,anchor:'95%'};
  ...
  //拣货出库
	...
	Fos.StorageNoteOutTab.superclass.constructor.call(this, {
	id: 'STORAGE_NOTE_'+p.get('uuid'),title:title,
	closable:true,layout:'border',autoScroll:true,
	tbar:
		[btnSave,'-',
...
Ext.extend(Fos.StorageNoteOutTab, Ext.Panel);
...
Fos.WNoteCargoOutWin = function(p,store) {
	...
	//数量单位
	var ccboUnitName=new Ext.form.ComboBox({fieldLabel:'EA单位',name:"unitName",value:p.get("unitName"),align:'center',tabIndex:72,
