...
//出库单费用审核
Fos.StorageNoteOutAuditGrid=function(){

	var aq=[];
	
	aq[aq.length]={key:'storageType',value:1,op:EQ};	//是出库的
	aq[aq.length]={key:'status',value:0,op:6};			//非新增的
	
	var bp={_mt:'json',xml:HTUtil.QATJ(aq)};
	
	var store=new Ext.data.GroupingStore({url:SERVICE_URL+'?_A=WStorageNoteExpenseCheck_Q',baseParams:bp,  
			reader:new Ext.data.JsonReader({totalProperty:'rowCount',root:'WStorageNote'},WStorageNote),
	...
	var cmIn=new Ext.grid.ColumnModel({columns:[new Ext.grid.RowNumberer(),sm,
			{header:"出库单号",width:130,dataIndex:'storageNoteNo'}
			,{header:'出库单状态',width:100,dataIndex:'status',align:'center',renderer:HTStore.getOutWarehouseNoteStatus},
			//审核状态
	...
	{
		var win = new Fos.StorageNoteLookupWin('WStorageNoteExpenseCheck_Q',store,1);
		win.show();
	}; 
	...
	this.fastSearch=function(){
		...
		if (c>=0)
		{
			a[a.length]={key:'storageNoteNo',value:storageNoteNo,op:IN};
		}
		else if(b>=0)
		{
			var ra=storageNoteNo.split('..');
			a[a.length]={key:'storageNoteNo',value:ra[0],op:GE};
			a[a.length]={key:'storageNoteNo',value:ra[1],op:LE};
		}
		
		a[a.length]={key:'storageNoteNo',value:storageNoteNo,op:LI};		//LI=7 like
...
//			}
   ...
   //closable 可关闭
	Fos.StorageNoteOutAuditGrid.superclass.constructor.call(this,{
			id:'SNOUTAU',iconCls:'grid',store:store,title:'出库单'+C_CONS_AUDIT,header:false,
			...
			listeners:{rowdblclick:function(g,r,e){
						...
						,
						rowclick:function(t,r,e)      //grid行单击选择，改变tbar上按钮的状态
						{
							var p=sm.getSelected();
							this.updateTB(p);
						}
...
Ext.extend(Fos.StorageNoteOutAuditGrid,Ext.grid.GridPanel);