...
Wms.InvoiceGrid = function(t) {
    ...
    this.showInvoice= function(p){
    	...
    	else {tab = this.ownerCt.add(new Wms.InvoiceTab(p));
	...
	this.add=function(){
		...
		w.addButton({text:C_OK,scope:this,handler:function(){
			...
			var tab = this.ownerCt.add(new Wms.InvoiceTab(e,store));
    ...
    Wms.InvoiceGrid.superclass.constructor.call(this, {
...
Ext.extend(Wms.InvoiceGrid, Ext.grid.GridPanel);

//应收、应付账单编辑窗口
Wms.InvoiceTab = function(p,listStore) {
    this.itemGrid=new Wms.InvoItemGrid(p,this);
    this.entryGrid=new Wms.InvoEntryGrid(p,this);  
    ...
    this.addInvoice=function(){
		...
		w.addButton({text:C_OK,scope:this,handler:function(){
			...
			var e = new SInvoice({invoNo:'N'+id,currCode:currCode,
				...
				version:'0',rowAction:'N',uuid:HTUtil.UUID(32)});
			var tab = this.ownerCt.add(new Wms.InvoiceTab(e));
			this.ownerCt.setActiveTab(tab);
		}},this);
		w.addButton({text:C_CANCEL,handler:function(){w.close();}},this);w.show();
    };
	...
	Wms.InvoiceTab.superclass.constructor.call(this, {
...
Ext.extend(Wms.InvoiceTab, Ext.FormPanel);


//发票-根据结算单位选择费用
Wms.ExpenseLookupWin = function(c,t) {
	...
	this.wmsSearch=function(){
      	var a=[];var op=1; 
    		...
    		var storageDate=this.find('name','storageDate')[0].getValue();
    		var storageDate2=this.find('name','storageDate2')[0].getValue();
    		if(storageDate && storageDate2){
    			a[a.length]=new QParam({key:'storageDate',value:storageDate.format(DATEF),op:5});
    			a[a.length]=new QParam({key:'storageDate',value:storageDate2.format(DATEF),op:3});    			
    		}
    		
    		var planedTime=this.find('name','planedTime')[0].getValue();
    		var planedTime2=this.find('name','planedTime2')[0].getValue();
    		if(planedTime && planedTime2){
    			a[a.length]=new QParam({key:'planedTime',value:planedTime.format(DATEF),op:5});
    			a[a.length]=new QParam({key:'planedTime',value:planedTime2.format(DATEF),op:3});    			
    		}
    		
    		var storageNo=this.find('name','storageNoteNo')[0].getValue();
    		if(storageNo){
    			a[a.length]=new QParam({key:'storageNoteNo',value:storageNo,op:LI});
    		}
    		
    		var orderNo=this.find('name','orderNo')[0].getValue();
    		if(orderNo){
	...
	var wmsForm = new Ext.form.FormPanel({region:'north',layout:'column',height:160,header:false,
		...
		{columnWidth:.34,layout:'form',border:false,items:[
        	{fieldLabel:"客户订单号",name:'orderNo',xtype:'textfield',anchor:'90%'}
        ]}],
    ...
    Wms.ExpenseLookupWin.superclass.constructor.call(this,{title:C_ADD_EXPE,modal:true,layout:'border',width:800,height:500,minWidth:300,
        minHeight:200,plain:false,bodyStyle:'padding:5px;',buttonAlign:'right',items:[wmsForm,g]}); 
};
Ext.extend(Wms.ExpenseLookupWin,Ext.Window);

//发票-费用明细
Wms.InvoItemGrid = function(p,frm){
	...
	this.add=function(){
		if(p.get('custId')){
    		var win = new Wms.ExpenseLookupWin(p.get('custId'),p.get('invoType'));
	...
	Wms.InvoItemGrid.superclass.constructor.call(this, {
...
Ext.extend(Wms.InvoItemGrid, Ext.grid.EditorGridPanel);

//发票-发票明细
Wms.InvoEntryGrid = function(p,frm) {
	...
	this.add=function(){
		...
		var e = new SInvoiceEntry({charName:'',unitName:'',inenNum:'',inenUnitPrice:'',inenTotalPrice:'',
			inenTotalAmount:'',currCode:'',inenExRate:'',inenInvoiceAmount:'',consBizType:'w',
			version:'0',rowAction:'N',uuid:HTUtil.UUID(32)});
	...
	Wms.InvoEntryGrid.superclass.constructor.call(this, {
...
Ext.extend(Wms.InvoEntryGrid, Ext.grid.EditorGridPanel);


//收款单、付款单列表
Wms.VoucherGrid = function(t){
	...
	this.showVoucher= function(p){
    	...
    	if(tab) {this.ownerCt.setActiveTab(tab);}
    		...
    		tab = this.ownerCt.add(new Wms.VoucherTab(p));
    		this.ownerCt.setActiveTab(tab);
    		tab.doLayout();
    	}
	};
	...
	this.add = function(){
		...
		w.addButton({text:C_OK,scope:this,handler:function(){
			...
			version:'0',rowAction:'N',uuid:HTUtil.UUID(32)});
			tab = this.ownerCt.add(new Wms.VoucherTab(e));
    		this.ownerCt.setActiveTab(tab);
		}},this);
		w.addButton({text:C_CANCEL,handler:function(){w.close();}},this);w.show();
	};
    ...
    Wms.VoucherGrid.superclass.constructor.call(this, {    
...
Ext.extend(Wms.VoucherGrid, Ext.grid.GridPanel);

//待核销项目查询
Wms.InitLookupWin = function(c,t) {	
    ...
    Wms.InitLookupWin.superclass.constructor.call(this,{title:C_ADD_EXPE,modal:true,layout:'fit',width:800,minWidth:300,
        minHeight:200,plain:false,bodyStyle:'padding:0px;',buttonAlign:'right',items:[{layout:'fit',border:false,
        items: [g]}]}); 
};
Ext.extend(Wms.InitLookupWin,Ext.Window);

//核销-核销明细
Wms.VoucItemGrid = function(p,store){
	var sm=new Ext.grid.CheckboxSelectionModel({singleSelect:false}); 
	var cm=new Ext.grid.ColumnModel({columns:[sm,
	...
	this.add=function(){
		if(p.get('custId')){
    		var win = new Wms.InitLookupWin(p.get('custId'),p.get('voucType'));
			win.addButton({text:C_OK,scope:this,handler:function(){
	...
	Wms.VoucItemGrid.superclass.constructor.call(this, {
...
Ext.extend(Wms.VoucItemGrid,Ext.grid.EditorGridPanel);

//收、付款核销编辑窗口，如果prId不为空，就是从付款申请直接付款核销
Wms.VoucherTab = function(p,prId) {
    ...
    this.grid=new Wms.VoucItemGrid(p,store);
    this.save=function(){
    	p.beginEdit();
    ...
    Wms.VoucherTab.superclass.constructor.call(this, {id:'T_VOUC_'+p.get('uuid'),layout:'border',
...
Ext.extend(Wms.VoucherTab,Ext.FormPanel);

Wms.PrGrid = function(t){
    ...
    Wms.PrGrid.superclass.constructor.call(this, {    
...
Ext.extend(Wms.PrGrid,Ext.grid.GridPanel);


Wms.PrItemGrid = function(p){
	...
	Wms.PrItemGrid.superclass.constructor.call(this,{id:'G_PR_I',border:false,autoScroll:true,clicksToEdit:1,store:store,sm:sm,cm:cm,height:350,
    listeners:{scope:this,afteredit:function(e){
    	var f=e.field;var r=e.record;
    	if(f=='prAmount'){
...
Ext.extend(Wms.PrItemGrid,Ext.grid.EditorGridPanel);


Wms.PrItemLookupWin = function(c,t,curr) {
	...
	Wms.PrItemLookupWin.superclass.constructor.call(this,{title:C_SEL_INVO,modal:true,layout:'fit',width:600,minWidth:300,
        minHeight:200,plain:false,bodyStyle:'padding:0px;',buttonAlign:'right',items:[{layout:'fit',border:false,
        items: [g]}]});
};
Ext.extend(Wms.PrItemLookupWin,Ext.Window);


Wms.PrTab = function(p) {
    ...
    Wms.PrTab.superclass.constructor.call(this, {
...
Ext.extend(Wms.PrTab, Ext.FormPanel);

Wms.InvoNoGrid = function() {
	...
	Wms.InvoNoGrid.superclass.constructor.call(this,{
...
Ext.extend(Wms.InvoNoGrid, Ext.grid.EditorGridPanel);


Wms.ExhiWin = function() {
...
Ext.extend(Wms.ExhiWin,Ext.Window);


Wms.ExhiGrid = function() {
	...
	Wms.ExhiGrid.superclass.constructor.call(this,{
...
Ext.extend(Wms.ExhiGrid, Ext.grid.GridPanel);

Wms.BalaGrid = function() {
	...
	Wms.BalaGrid.superclass.constructor.call(this,{
...
Ext.extend(Wms.BalaGrid, Ext.grid.GridPanel);


Wms.BaliWin = function(b){
    var store = new Ext.data.Store({url:SERVICE_URL,baseParams:{_A:'VOUC_X',_mt:'json'},
		reader:new Ext.data.JsonReader({totalProperty:'rowCount',root:'SVoucher',id:'id'},SVoucher),
		remoteSort:true,sortInfo:{field:'id', direction:'desc'}});	
    ...
    Wms.BaliWin.superclass.constructor.call(this, {title:C_FIX_LIST,modal:true,width:600,
        height:400,plain:false,bodyStyle:'padding:0px;',buttonAlign:'right',layout:'fit',items:g}); 
};
Ext.extend(Wms.BaliWin,Ext.Window);

Wms.ExalWin = function(store,p) {
	var sm=new Ext.grid.CheckboxSelectionModel({singleSelect:false});
	var cm=new Ext.grid.ColumnModel({columns:[
			{header:C_CONS_NO,width:120,dataIndex:"consNo"},
	...
	Wms.ExalWin.superclass.constructor.call(this, {title:C_EXPE_ALLOCATION,modal:true,width:800,height:600,maximizable:true,
		plain:false,bodyStyle:'padding:0px;',buttonAlign:'right',layout:'border',items:[expeGrid,consGrid,exalGrid
		    ]}); 
};
Ext.extend(Wms.ExalWin, Ext.Window);

Wms.CurrencyWin = function(){
	Wms.CurrencyWin.superclass.constructor.call(this, 
...
Ext.extend(Wms.CurrencyWin, Ext.Window);

//账单查询
Wms.InvoLookupWin = function(t) {
	...
	Wms.InvoLookupWin.superclass.constructor.call(this, {title:C_INVO_QUERY,iconCls:'search',modal:true,width:600,minWidth:300,
        minHeight:200,plain:true,bodyStyle:'padding:0px;',buttonAlign:'right',items:frmLookup}); 
};
Ext.extend(Wms.InvoLookupWin, Ext.Window);

Wms.PrLookupWin = function(t) {
	var frmLookup = new Ext.form.FormPanel({labelWidth:70,labelAlign:"right",
	...
	Wms.PrLookupWin.superclass.constructor.call(this, {title:t=='R'?C_PR_R_QUERY:C_PR_P_QUERY,iconCls:'search',modal:true,width:600,minWidth:300,
        minHeight:200,plain:true,bodyStyle:'padding:0px;',buttonAlign:'right',items:frmLookup}); 
};
Ext.extend(Wms.PrLookupWin, Ext.Window);

//核销查询
Wms.VoucLookupWin = function(t) {
	var frmLookup = new Ext.form.FormPanel({labelWidth:70,labelAlign:"right",
	...
	Wms.VoucLookupWin.superclass.constructor.call(this, {title:t=='R'?C_VOUC_R_QUERY:C_VOUC_P_QUERY,iconCls:'search',modal:true,width:600,minWidth:300,
        minHeight:200,plain:true,bodyStyle:'padding:0px;',buttonAlign:'right',items:frmLookup}); 
};
Ext.extend(Wms.VoucLookupWin,Ext.Window);

Wms.BillLookupWin = function(store,t) {
	...
	Wms.BillLookupWin.superclass.constructor.call(this, {title:C_BILL_QUERY,iconCls:'search',modal:true,width:600,height:300,minWidth:300,
...
Ext.extend(Wms.BillLookupWin, Ext.Window);

Wms.ExSearchWin = function(p,t) {
	this.store = new Ext.data.Store({url:SERVICE_URL,baseParams:{_A:'M_OR_A_X',_mt:'xml'},
		reader:new Ext.data.XmlReader({totalProperty:'rowCount',record:'SExpense',id:'id'},SExpense),
		remoteSort:true,sortInfo:{field:'id', direction:'desc'}});	
    this.store.load({params:{custId:p.get('custId'),expeType:t,expeBillStatus:'0'}});
       
	this.wmsSearch=function(){
      	var a=[];var op=1; 
      		a[a.length] = new QParam({key:'consBizType',value:'W',op:op});
    		var wmsCustId=this.find('name','wmsCustId');
    		if(wmsCustId.length>0) 
    			a[a.length]=new QParam({key:'custId',value:wmsCustId[0].getValue(),op:op});
    		var sddF=storageDateF.value;
    		if(sddF)
    			a[a.length]=new QParam({key:'storageDate',value:sddF,op:GE});
    		var sddE=storageDateE.value;
    		if(sddE){
    			a[a.length]=new QParam({key:'storageDate',value:sddE,op:LE});    			
    		}
    		var actTF=actureTimeF.value;
    		if(actTF)
    			a[a.length]=new QParam({key:'actureTime',value:actTF,op:GE});
    		var actTE=actureTimeE.value;
    		if(actTE)
    			a[a.length]=new QParam({key:'actureTiem',value:actTE,op:LE});
    		if(storageNoteNo.getValue())
    			a[a.length]=new QParam({key:'storageNoteNo',value:storageNoteNo.getValue(),op:LI});
    		if(orderNo.getValue())
    			a[a.length]=new QParam({key:'orderNo',value:orderNo.getValue(),op:LI});
	...
	var wmsCustName=new Ext.form.TextField({fieldLabel:'单称名称',name:'wmsCustName',value:p.get('custName'),anchor:'94.7%'});
	var wmsCustId=new Fos.CustomerLookup({fieldLabel:C_SETTLE_OBJECT,tabIndex:1,name:'wmsCustId',value:p.get('custId'),
    	...
    	listeners:{scope:this,
    		...
    		select:function(c,r,i){
    			...
    			wmsCustName.setValue(r.get('custNameCn'));
    		}}});
	var storageDateF=new Ext.form.DateField({fieldLabel:C_STORAGE_DATE,tabIndex:17,name:'storageDate',xtype:'datefield',format:DATEF,anchor:'90%'});
	var storageDateE=new Ext.form.DateField({fieldLabel:C_TO,tabIndex:18,name:'storageDate2',xtype:'datefield',format:DATEF,anchor:'90%'});
	var actureTimeF=new Ext.form.DateField({fieldLabel:'完成时间',tabIndex:19,name:'actureTime',xtype:'datefield',format:DATEF,anchor:'90%'});
	var actureTimeE=new Ext.form.DateField({fieldLabel:C_TO,tabIndex:19,name:'actureTime2',xtype:'datefield',format:DATEF,anchor:'90%'});
	var storageNoteNo=new Ext.form.TextField({fieldLabel:'仓单号',tabIndex:13,name:'storageNoteNo',xtype:'textfield',anchor:'90%'});
	var orderNo=new Ext.form.TextField({fieldLabel:"订单号",name:'orderNo',xtype:'textfield',anchor:'90%'});
	
	var wmsForm = new Ext.form.FormPanel({layout:'column',height:140,region:'north',header:false,
		layoutConfig:{columns:3},collapsible:true,labelAlign:'right',activeItem:5,padding:5,
		items:[
		       {columnWidth:.33,layout:'form',labelWidth:70,border:false,
		    	  items:[wmsCustId]},
	           {columnWidth:.67,layout:'form',labelWidth:70,border:false,
		          items:[wmsCustName]},
			   {columnWidth:.33,layout:'form',labelWidth:70,border:false,
			      items:[storageDateF,actureTimeF]},
    	       {columnWidth:.33,layout:'form',labelWidth:70,border:false,
			      items:[storageDateE,actureTimeE]},
		       {columnWidth:.34,layout:'form',labelWidth:80,border:false,
			      items:[storageNoteNo,orderNo]}],
        buttons:[{text:C_SEARCH,iconCls:'refresh',scope:this,handler:this.wmsSearch},{text:C_CLEAR_FILTER,iconCls:'rotate',scope:this,handler:this.wmsClear}]});
    ...
    Wms.ExSearchWin.superclass.constructor.call(this,{title:C_EXPE_QUERY,modal:true,width:900,
    	height:500,buttonAlign:'right',layout:'border',
        items:[wmsForm,this.grid]
    }); 
};
Ext.extend(Wms.ExSearchWin,Ext.Window);

/*
 * 对账单列表
 * t 对账单类型，R：应收  P：应付
 */
Wms.BillGrid = function(t) {
    ...
    this.showBill = function(p){
		...
		if(tab){
			...
			tab = this.ownerCt.add(new Wms.BillTab(p));
			this.ownerCt.setActiveTab(tab);
			tab.doLayout();
		}
	};
	...
	this.addBill = function(){
		...
		var tab = this.ownerCt.add(new Wms.BillTab(r));
		this.ownerCt.setActiveTab(tab);
    };
    ...
    Wms.BillGrid.superclass.constructor.call(this, {    
...
Ext.extend(Wms.BillGrid,Ext.grid.GridPanel);

Wms.BillTab = function(p){
    ...
    var addExp=function(){
    	...
    	if(custId){
    		var win = new Wms.ExSearchWin(p,p.get('billType'));
			win.addButton({text:C_OK,scope:this,handler:function(){
				var g = win.grid;
				var r = g.getSelectionModel().getSelections();
				if(r){
    ...
    Wms.BillTab.superclass.constructor.call(this, { 
...
Ext.extend(Wms.BillTab, Ext.FormPanel);

//费用模板
Wms.ExpenseTemplateGrid = function() {
	...
	Wms.ExpenseTemplateGrid.superclass.constructor.call(this, {title:C_EXPENSE_TEMPLATE,
...
Ext.extend(Wms.ExpenseTemplateGrid, Ext.grid.EditorGridPanel);

//费用模板明细
Wms.ExpenseTemplateItemGrid = function(p) {
	...
	Wms.ExpenseTemplateItemGrid.superclass.constructor.call(this, {title:C_EXPENSE_TEMPLATE,
		autoScroll:true,clicksToEdit:1,height:250,closable:true,
		stripeRows:true,store:store,sm:sm,cm:cm,
		tbar:[btnAdd, '-',btnRemove,'-',btnSave]
	});
};
Ext.extend(Wms.ExpenseTemplateItemGrid, Ext.grid.EditorGridPanel);
