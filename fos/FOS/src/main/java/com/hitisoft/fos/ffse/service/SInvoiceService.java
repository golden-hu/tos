...
public class SInvoiceService {
	...
	@Autowired
	private TExpressDao tExpressDao;
	@Autowired
	...
	public List save(List entityList) {
		...
		for (String key : keySet) {
			...
			if(bizType.equals(Constants.CONS_BIZ_TYPE_TMS)){
			...
			}else if(bizType.equals(Constants.CONS_BIZ_TYPE_EXPRESS)){
				TExpress fe = null;
				TExpress exp = tExpressDao.findById(Long.parseLong(consId));
				syncConsignEMS(exp);
				tExpressDao.update(exp);
				if (fe == null || fe.getId() > exp.getId()) {
					fe = exp;
				}
				if (!isInvoDelFlag && fe != null) {
					if (retInvoice == null) {
						retInvoice = dao.findById(parentId);
					}
					dao.update(retInvoice);
				...
				FConsign fc = null;
				FConsign consign = consignDao.findById(Long.parseLong(consId));
				syncConsign(consign);
				consignDao.update(consign);
				if (fc == null || fc.getId() > consign.getId()) {
					fc = consign;
				}
				...
				if (!isInvoDelFlag && fc != null) {
					...
					copyInfoFromConsign(retInvoice, fc);
					dao.update(retInvoice);
				}
			}
		}
	...
	/**
	 * 开票, 需要更新委托的开票状态和开票时间‘EMS’
	 */
	@Transactional
	private void syncConsignEMS(TExpress consign) {
		List<SExpense> expenseList = expenseDao.findByProperty("consId", "" + consign.getId());
		Byte statusAr = Constants.EXPENSE_INVOICE_STATUS_NONE;
		Byte statusAp = Constants.EXPENSE_INVOICE_STATUS_NONE;
		...
		consign.setConsStatusInvoR(statusAr.intValue());
		...
		consign.setConsStatusInvoP(statusAp.intValue());
	}
